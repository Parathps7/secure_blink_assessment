...
const register = asyncHandler(async(req,res) => {
    ...
    if( !username || !email || !password ){
        res.status(404).send({success:false,msg:"Please enter all credentials(username,email,password)"});
        throw new Error("All fields are mandatory");
    }
...
const forgetpassword = asyncHandler(async(req,res) => {
    ...
    res.status(200).send({success:true,msg:`Reset email sent to ${req.body.email}.Please check Inbox!.Make POST request with "password"=<new_password> in body`})
